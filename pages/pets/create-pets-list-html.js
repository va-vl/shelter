...
while (passCounter < PASS_TARGET) {
  ...
  while (tempTags.size < TAGS_AMOUNT) {
    const getFillCondition = () => forbiddenTags.includes(tag)
      && tempTags.size < (CARDS_PER_SCREEN - forbiddenAmount);
    let tag;
...
slideshowList.innerHTML = resultTags.reduce((acc, tag) => `${acc}${createCardHTML(parseTag(tag))}`, '');
