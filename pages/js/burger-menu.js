...
async function openMenu() {
  if (!isMoving && !header.classList.contains("page-header--menu-open")) {
    ...
    await playAnimationOnce(
      headerWrapper,
      "slideBurgerMenu 600ms ease forwards"
    );
...
async function closeMenu() {
  if (!isMoving && header.classList.contains("page-header--menu-open")) {
    ...
    await playAnimationOnce(
      menuButton,
      "turn 200ms ease 200ms reverse forwards"
    );
    await playAnimationOnce(
      headerMenuWrapper,
      "fade 200ms ease reverse forwards"
    );
    await playAnimationOnce(
      headerWrapper,
      "slideBurgerMenu 400ms ease reverse forwards"
    );
...
window.addEventListener("resize", (event) => {
  ...
  if (
    ...
    header.classList.contains("page-header--menu-open")
  ) {
    header.classList.remove("page-header--menu-open");
